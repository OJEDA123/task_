    ...
    && window.localStorage.getItem('tecnicos')){
        ...
        for (i = 0; i < 100; i++) {
            if(document.getElementsByName('flexCheckChecked')[i]){
                if(document.getElementsByName('flexCheckChecked')[i].checked== true){
                    task_order.push(document.getElementsByName('flexCheckChecked')[i].value);
                }

            }
        }
        console.log(task_order[0])
...
function checkAll(w_order,val) {
    if(val == 1){
        ...
        document.querySelectorAll('#grupob input[type=checkbox]').forEach(function(checkElement) {
            checkElement.checked = true;
        });
        same_local_storage_task(w_order);
    }else if(val == 3){
        document.querySelectorAll('#grupoverify input[type=checkbox]').forEach(function(checkElement) {
            checkElement.checked = true;
        });
        same_local_storage_task(w_order);
    }else if(val == 4){
        document.querySelectorAll('#grupoins input[type=checkbox]').forEach(function(checkElement) {
            checkElement.checked = true;
        });
        same_local_storage_task(w_order);
    }
